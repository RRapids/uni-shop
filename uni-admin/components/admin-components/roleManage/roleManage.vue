	...
	<view class="scroll_container flex_center">
		<view class="scroll_card">
			...
			<uni-table border stripe emptyText="暂无更多数据" type="selection" @selection-change="change">
				...
				<uni-tr v-for="(item,index) in (RoleListData ? RoleListData : PersonList)" :key='index'>
			...
			<view class="fenyeStyle">
				<uni-pagination @change="handlePage" show-icon="true" :total="(role_total ? role_total : total)"
			...
			<view class="popupTop">
				...
				<view class="popupCenter">
					<view class="center">
						...
						角色
						<input v-model="form.roleName" class="input_account" />
					</view>
					<view class="center">
						<view style="color: red;">*</view>
						备注
						<input v-model="form.note" class="input_account" />
					</view>
				</view>
				<!-- 下 -->
...
<script>
	...
	export default {
		...
		onLoad() {
			this.RoleListData = this.PersonList
		...
		methods: {
			...
			getRoleList() {
				uniCloud
					...
					.then(res => {
						...
						this.RoleListData = res.result.res.data
			...
			add_data() {
				this.form = {
					id: '',
					roleId: this.PersonList.length + 1,
					roleName: '',
					permission: '',
					createTime: '',
					note: ''
					...
					uni.showModal({
						...
						success: (res) => {
							if (res.confirm) {
								...
								//实现全选删除和多选删除
								let arr = [];
								let len = this.RoleListData.length;

								// 还未执行其余操作，就直接多选，此时RoleListData还未赋值		
								if (len == 0 || len == "") {
								...
								} else { // 点击分页后，RoleListData有值
									for (let i = 0; i < len; i++) {
										if (this.selectArr.indexOf(i) >= 0) {
											arr.push({
												id: this.RoleListData[i]._id
											})
										}
									}
